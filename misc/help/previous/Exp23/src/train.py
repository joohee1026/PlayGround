...
from networks import EffB4Unetpp_aux
...
def train():
    ...
    model = EffB4Unetpp_aux((IMAGE_SIZE, IMAGE_SIZE, 1), ch=CH)
    model.load_weights("/data/volume/sinyu/Exp23/100_1.20120_0.04281_0.60449.h5")

    CB = CustomCallbacks(log_dir=LOG_DIR, nb_epochs=EPOCHS, nb_snapshots=1, init_lr=LR)

    model.compile(
        optimizer=Adam(),
        loss=[cce_dice_loss, cce_dice_loss, cce_dice_loss, cce_dice_loss],
