...
def ConvBlockAttModule(inputs, r=8, k_size=7, only_ch=True):
...
def CUnet_light(input_shape=(None, None, 1), k=3, ch=16, n_classes=8):
    inputs = Input(input_shape)
    # contracting path
    x = layers.Conv2D(ch*4, 7, 2, "same")(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.LeakyReLU()(x)
    x1, skip1 = CUnetConv(x, k, ch*4, True)
    x2 = layers.Conv2D(ch*4, k, 2, "same")(x1)
    ...
    x4, skip4 = CUnetConv(x4, k, ch*32, True)
    x5 = layers.Conv2D(ch * 16, k, 2, "same")(x4)
    ...
    x10 = layers.Conv2DTranspose(ch*2, 7, 2, "same")(x9)
    x10 = layers.BatchNormalization()(x10)
    x10 = layers.LeakyReLU()(x10)

    outputs = layers.Conv2D(n_classes, 1, 1, activation="sigmoid")(x10)
    return Model(inputs, outputs)


def CoCUnet(input_shape=(None,None,1), k=3, ch=16, n_classes=8):
    input_shape_c = (input_shape[0], input_shape[1], input_shape[2]+2)
    bm = CUnet(input_shape_c, k, ch, n_classes)
...
def CoEffB4Unetpp(input_shape=(None,None,1), ch=16, n_classes=9):
    input_shape_c = (input_shape[0], input_shape[1], input_shape[2]+2)
    bm = EffB4Unetpp_v2(input_shape_c, ch, n_classes=n_classes)
