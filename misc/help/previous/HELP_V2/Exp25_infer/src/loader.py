...
class RoiDataLoader(Sequence):
    def __init__(self,
        ...
        super(RoiDataLoader, self).__init__()
        assert mode in ["train", "valid"]
...
#     (e1, e2), (e3, e4) = get_roi(y)
#     if rand:
#         v1 = np.random.randint(0, e1//1.2, 1)[0]
#         v3 = np.random.randint((y.shape[0]-e3)//1.2, y.shape[0]-e3, 1)[0]
#         v2 = np.random.randint(0, e2//1.2, 1)[0]
#         v4 = np.random.randint((y.shape[1]-e4)//1.2, y.shape[1] - e4, 1)[0]
#
#         return x[(e1-v1):(e3+v3), (e2-v2):(e4+v4)], y[(e1-v1):(e3+v3), (e2-v2):(e4+v4), :]
#     else:
#         return x[e1-16:e3+16, e2-16:e4+16], y[e1-16:e3+16, e2-16:e4+16, :]
