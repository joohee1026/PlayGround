...
from losses import cce_dice_focal_loss, dice_loss, iou_score, focal_loss
...
def train():
    ...
    # model.load_weights("/data/volume/sinyu/Exp15/093_0.30749_0.60239.h5")

    CB = CustomCallbacks(log_dir=LOG_DIR, nb_epochs=EPOCHS, nb_snapshots=1, init_lr=LR)

    model.compile(
        optimizer=Adam(),
        loss=cce_dice_focal_loss,
        metrics=['categorical_crossentropy', dice_loss, focal_loss, iou_score]
    )
