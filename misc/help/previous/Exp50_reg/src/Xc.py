...
def AXUNetpp(input_shape=(None, None, 1), ch=16, n_classes=9):
    ...
    out = layers.Conv2D(ch*2, 3, 2, "same", use_bias=False)(inputs)
    out = layers.BatchNormalization()(out)
    out = layers.LeakyReLU()(out)

    out = ConvBlock(out, ch*4, 3, 1)
    out = layers.BatchNormalization()(out)
    out = layers.LeakyReLU()(out)

    out = XceptionBlock(out, [128,128,128], "conv", 2, 1, False)
    out, skip1 = XceptionBlock(out, [256,256,256], "conv", 2, 1, False, True)
    out = XceptionBlock(out, [512,512,728], "conv", entry_block3_stride, 1, False)
    for _ in range(8):
        out = XceptionBlock(out, [728,728,728], "sum", 1, middle_block_rate, depth_activation=False)
