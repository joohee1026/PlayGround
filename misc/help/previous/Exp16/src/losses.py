...
def cce_dice_focal_loss_(y_true, y_pred):
    return 5*losses.categorical_crossentropy(y_true, y_pred) + (1 - f_score(y_true, y_pred)) + categorical_focal_loss(y_true, y_pred)
...
def f_score(gt, pr, beta=1, class_weights=1, class_indexes=None, smooth=1e-5, per_image=False, threshold=None):
    ...
    # tp = K.clip(tp, K.epsilon(), 1 - K.epsilon())
    # fp = K.clip(fp, K.epsilon(), 1 - K.epsilon())
    # fn = K.clip(fn, K.epsilon(), 1 - K.epsilon())
