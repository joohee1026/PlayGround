...
class DataLoader_v2(Sequence):
    def __init__(self,
        ...
        for i in range(len(self.input_paths)):
            img = pydicom.dcmread(self.input_paths[i]).pixel_array
            img = normalize(cv2.resize(img, (img_size, img_size))).astype(np.float32)
            self.images.append(img)

            mask = [np.array(cv2.imread(i, cv2.IMREAD_GRAYSCALE)) for i in self.target_paths[i]]
            mask = normalize(np.stack(mask, axis=-1)).astype(np.float32)
    ...
    def __getitem__(self, idx):
        ...
        for i in indexes:
            ...
            if self.augmentation:
                imgae, mask = rotate_90(image, mask)
                aug = self.augmentation(image=image, mask=mask)
                image = normalize(aug["image"])[..., np.newaxis]
                mask = normalize(aug["mask"])
                bx.append(image)
