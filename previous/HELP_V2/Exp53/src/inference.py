...
def inference(REG_DIR=None, SEG_DIR=None):
    ...
    # W_path = "/data/volume/sinyu/Exp53_1024"
    # W = sorted(glob(join(W_path, "*.h5")))[-1]
    W = "/data/volume/sinyu/Exp53_1024/009_0.19527_0.72793.h5"

    model = CoEffB4Unetpp((IMAGE_SIZE, IMAGE_SIZE, 19), ch=CH, n_classes=9)
    model.load_weights(W)
    print("Load ", W)
    ...
    for i, f in enumerate(test_files):
        ...
        pred = (np.arange(pred.max()) == pred[..., None]).astype(np.float32)
        ...
        for i in range(8):
            pred_i = pred[..., i]
            upsample_img = cv2.resize(pred_i, (w,h))
