...
def AttAugmentationModule(x, ch=None, k=3, s=1, depth_k=8, depth_v=8, num_heads=4):
    if ch is None:
        ch = x._shape_tuple()[-1]
    # depth_k, depth_v = normalize_depth_vars(depth_k, depth_v, ch)
...
def TransposeConvBlock(x, ch, n, module="aa"):
    if module == "cbam":
        x = ConvBlockAttModule(x)
    if module == "aa":
        x = AttAugmentationModule(x)
...
def EffB4Unetpp_v2(input_shape=(None, None, 1), ch=16, module="aa", n_classes=9):
    """
    Difference from 'EffB4Unetpp' is that first layer of the decoder, pooling deleted and CBAM or AA module added
    """
